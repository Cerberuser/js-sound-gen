...
class Wav {
    ...
    private _bufferNeedle: number;
    private _buffer: Int16Array | undefined;
    private _internalBuffer: string | undefined;
    private _hasOutputHeader: boolean | undefined;

    constructor(opt_params: Partial<WavProps>) {
    ...
    getBuffer(len: number) {
        if (this._buffer) {
            ...
            throw new Error("No buffer to get");
        }
    };
    ...
    getWavInt16Array(buffer: Float32Array) {

        let intBuffer = new Int16Array(buffer.length + 23), tmp;
...
export function make_wav(sampleRate: number, buffer: Float32Array) {
